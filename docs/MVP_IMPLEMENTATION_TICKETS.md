...
- Completed: T2.4
- Completed: T3.1
- Next up: T3.2 global shortcut system with fn-default + fallback
...
**Progress Notes:**
- ✅ Added desktop app controller scaffold in `apps/desktop/src/vaultvoice_desktop/app_controller.py` to wire push-to-talk lifecycle calls to local-service start/stream/finalize methods.
- ✅ Added in-memory live/final transcript state management and explicit dictation states (`idle`, `listening`, `processing`, `error`).
- ✅ Added desktop controller tests covering push-to-talk lifecycle and error transitions.
